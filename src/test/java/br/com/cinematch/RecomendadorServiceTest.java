...
class RecomendadorServiceTest extends BaseTeste {
    ...
    void deve_RespeitarTopN_Quando_Recomendar() {
        when(catalogo.buscarTodos()).thenReturn(List.of(chegada(), duna(), click()));
    ...
    @Test
    @DisplayName("deve retornar lista vazia quando catálogo lançar IOException simulada")
    void deve_RetornarListaVazia_Quando_CatalogoLancaIOExceptionSimulada() {
        when(catalogo.buscarTodos()).thenThrow(new RuntimeException("IOException: API offline"));
    ...
    @Test
    @DisplayName("deve lançar exceção quando topN é inválido")
    void deve_LancarExcecao_Quando_TopNInvalido() {
        assertThrows(IllegalArgumentException.class, () -> service.recomendar(usuario, 0));
    }
